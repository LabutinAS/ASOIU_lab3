...
void __fastcall TForm8::SetData(TADOTable *Table)
{
	...
	DBLookupComboBox1->ListField = "Название"; // Поле, отображающее ФИО
	DBLookupComboBox1->KeyField = "Номер_тренировки"; // Поле, содержащее ID сотрудника
	DBLookupComboBox1->KeyValue = Table->FieldByName("Номер_тренировки")->AsInteger; // Установите текущее значение
	...
	DBLookupComboBox2->KeyField = "Номер_клиента"; // Поле, содержащее ID клиента
	DBLookupComboBox2->KeyValue = Table->FieldByName("Номер_клиента")->AsInteger; // Установите текущее значение
}
 ...
 try
	{
		...
		ADOTable3->FieldByName("Номер_тренировки")->AsInteger = selectedID;
		int selectedID2 = DBLookupComboBox2->KeyValue;
		ADOTable3->FieldByName("Номер_клиента")->AsInteger = selectedID2;
